...
class Config:
    ...
    # His timeframe hierarchy, in his order: the 4-hour and 1-hour give the
    # bias because that is where the trend lives, the 15-minute breaks it down,
    # the 5-minute is where break of structure and gaps are read, and the
    # 1-minute is only the entry. The engine read the 4-hour, the 5-minute and
    # the 1-minute, and consulted neither the 1-hour nor the 15-minute at all.
    # Factors are in 5-minute bars: 12 = one hour, 3 = fifteen minutes.
    use_h1: bool = True             # measure the 1-hour trend
    use_m15: bool = True            # measure the 15-minute trend
    h1_factor: int = 12
    m15_factor: int = 3
    require_h1_align: bool = False  # refuse trades against the 1-hour
    require_m15_align: bool = False # refuse trades against the 15-minute
...
def find_setups(df, cfg=None, session_filter=None, smt_df=None):
    ...
    for i in range(start, n - 1):
        ...
        for (sbar, skind, slevel, sextreme) in list(recent):
            ...
            # His hierarchy: the 4-hour and 1-hour give the bias, the 15-minute
            # breaks it down. Tagged rather than required, so each layer can be
            # measured before anything is gated on it. htf_trend earned its
            # place that way; these have not yet.
            if h1_at(i) == direction:
                tags.append("h1_trend")                  # 1-hour agrees
            if m15_at(i) == direction:
                tags.append("m15_trend")                 # 15-minute agrees
            if cfg.require_h1_align and h1_at(i) not in (direction, None):
                continue
            if cfg.require_m15_align and m15_at(i) not in (direction, None):
                continue
            ...
            if cfg.require_index_align:
                ...
                same = any(b in pair_bos[direction] for b in range(lo_k, hi_k))
                other = "bull" if direction == "bear" else "bear"
                opposed = any(b in pair_bos[other] for b in range(lo_k, hi_k))
                # SILENCE IS NOT DISAGREEMENT. His rule refuses trades when the
                # two indexes CONTRADICT each other ("if ES is bullish and
                # NASDAQ is bearish I don't want to take the trade"), not when
                # one of them simply has not set up. On 28 May he takes the
                # NASDAQ trade precisely because ES did not get the
                # manipulation. Requiring the pair to confirm was vetoing the
                # trades he actually takes.
                if opposed and not same:
                    continue
